## slicepy
[![Build Status](https://travis-ci.org/uwescience/slicepy.svg?branch=master)](https://travis-ci.org/uwescience/slicepy)
...
instructions at the [bottom of this page](#using-slicepy-as-a-template).
...
`slicepy/slicepy.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `slicepy.py` in directory called
`slicepy`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import slicepy as sb` in an
interactive Python session, the classes and functions defined inside of the
`slicepy.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the slicepy module, the main classes defined at the bottom of
...
Either way, you can create a `slicepy/data` folder in which you can
...
(typically, something like our `slicepy/tests/test_slicepy.py`). Within each
...
have one such test in `slicepy/tests/test_slicepy.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/slicepy`),
and type:
...
will see a message such as:
    ...
    slicepy/tests/test_slicepy.py .F...
    ...
    slicepy/tests/test_slicepy.py:49: AssertionError
...
To document `slicepy` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/slicepy/blob/master/doc/_static/RTD-advanced-conf.png)

 http://slicepy.readthedocs.org/en/latest/
...
A `slicepy/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/slicepy) for the software. This
...
`slicepy` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `slicepy`, we use the
...
[here](https://github.com/uwescience/slicepy/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/slicepy
...
`slicepy/slicepy.py` to be called `smallish/smallish.py`

	git mv slicepy smallish
	git mv smallish/slicepy.py smallish/smallish.py
	git mv smallish/tests/test_slicepy.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `slicepy` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `slicepy` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `slicepy` in them. Type
the following to see where all these files are:

	git grep slicepy

You can replace `slicepy` for `smallish` quickly with:

	git grep -l 'slicepy' | xargs sed -i 's/slicepy/smallish/g'
...
Edit all the mentions of `slicepy` in the `slicepy/__init__.py` file, and in
the `slicepy/version.py` file as well.
